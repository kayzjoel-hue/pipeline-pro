...
export async function getUserInfo(): Promise<User | null> {
  try {
    ...
    const raw = JSON.parse(info) as Partial<User> & { lastSignedIn?: string };
    return {
      id: typeof raw.id === "number" ? raw.id : Date.now(),
      openId: typeof raw.openId === "string" ? raw.openId : `local-${Date.now()}`,
      name: raw.name ?? null,
      email: raw.email ?? null,
      loginMethod: raw.loginMethod ?? "local",
      lastSignedIn: raw.lastSignedIn ? new Date(raw.lastSignedIn) : new Date(),
    };
  } catch (error) {
